...
type
  ...
  public
    ...
    //10
    procedure OverlayCanvas(source: T23030428Canvas);
  end;
...
//10
procedure T23030428Canvas.OverlayCanvas(source: T23030428Canvas);
var
  i, j: integer;
  destX, destY: integer;
  srcColor: TRGB;
  transparent: TRGB;
begin
  transparent := self.pencil.color;

  for i := 0 to source.GetHeight - 1 do
    for j := 0 to source.GetWidth - 1 do
    begin
      srcColor := source.pixels[i][j];
      if (srcColor.R = transparent.R) and
         (srcColor.G = transparent.G) and
         (srcColor.B = transparent.B) then
         continue;
