...
begin
  ...
  begin
    ...
    if res = 0 then
    begin
      // Найдено вхождение, расширяем диапазон влево
      L := mid - 1;
      while (L >= 0) and (comp(data[L]) = 0) do
        Dec(L);
      Inc(L); // L теперь — первая подходящая позиция

      // Расширяем диапазон вправо
      R := mid + 1;
      while (R < length(data)) and (comp(data[R]) = 0) do
        Inc(R);
      Dec(R); // R теперь — последняя подходящая позиция

      // Копируем индексы L..R в результат
